...
"rtl" == $("html").data("textdirection") && (direction = "rtl"), "laravel" === $("body").attr("data-framework") && (assetPath = $("body").attr("data-asset-path")), $(document).on("click", ".fc-sidebarToggle-button", function (e) {
    ...
    var e, t = document.getElementById("calendar"), a = $(".event-sidebar"), n = {Business: "primary", Holiday: "success", Personal: "danger", Family: "warning", ETC: "info"}, l = $(".event-form"), d = $(".add-event-btn"), o = $(".btn-cancel"),
        r = $(".update-event-btn"), i = $(".btn-toggle-sidebar"), s = $("#title"), c = $("#select-label"), v = $("#start-date"), p = $("#end-date"), u = $("#event-url"), f = $("#event-guests"), h = $("#event-location"), m = $(".allDay-switch"),
        g = $(".select-all"), b = $(".calendar-events-filter"), y = $(".input-filter"), k = $(".btn-delete-event"), C = $("#event-description-editor");
    if ($(".add-event button").on("click", function (e) {
        ...
        c.wrap('<div class="position-relative"></div>').select2({
            placeholder: "Select value", dropdownParent: c.parent(), templateResult: w, templateSelection: w, minimumResultsForSearch: -1, escapeMarkup: function (e) {
                return e
            }
        })
    }
    if (f.length) {
        function x(e) {
            return e.id ? "<div class='d-flex flex-wrap align-items-center'><div class='avatar avatar-sm my-0 mr-50'><span class='avatar-content'><img src='" + assetPath + "images/avatars/" + $(e.element).data("avatar") + "' alt='avatar' /></span></div>" + e.text + "</div>" : e.text
        }

        f.wrap('<div class="position-relative"></div>').select2({
            placeholder: "Select value", dropdownParent: f.parent(), closeOnSelect: !1, templateResult: x, templateSelection: x, escapeMarkup: function (e) {
                return e
            }
        })
    }
    if (v.length) var P = v.flatpickr({
        enableTime: !0, altFormat: "Y-m-dTH:i:S", onReady: function (e, t, a) {
            a.isMobile && $(a.mobileInput).attr("step", null)
        }
    });
    if (p.length) var D = p.flatpickr({
        enableTime: !0, altFormat: "Y-m-dTH:i:S", onReady: function (e, t, a) {
            a.isMobile && $(a.mobileInput).attr("step", null)
        }
    });
        ...
        events: function (e, t) {
            ...
            events = $.ajax({
                ...
                success: function (result) {
                    ...
                    // events = [result.events.filter(event => n.includes(e.extendedProps.calendar.toLowerCase()))];
...
console.log(result.events);
        ...
        dateClick: function (e) {
            ...
            T(), a.modal("show"), d.removeClass("d-none"), r.addClass("d-none"), k.addClass("d-none"), v.val(t), p.val(t)
        },
    ...
    E.render(), S(), l.length && l.validate({
        ...
        }, title: {required: !0}, "start-date": {required: !0}, "end-date": {required: !0}
    ...
    function T() {
        p.val(""), u.val(""), v.val(""), s.val(""), h.val(""), m.prop("checked", !1), f.val("").trigger("change"), C.val("")
    }

    $(d).on("click", function () {
        if (l.valid()) {
            var e = {id: E.getEvents().length + 1, title: s.val(), start: v.val(), end: p.val(), startStr: v.val(), endStr: p.val(), display: "block", extendedProps: {location: h.val(), guests: f.val(), calendar: c.val(), description: C.val()}};
            u.val().length && (e.url = u.val()), m.prop("checked") && (e.allDay = !0), t = e, E.addEvent(t), E.refetchEvents()
        }
        var t
    }), r.on("click", function () {
        l.valid() && (!function (e) {
        ...
        k.addClass("d-none"), r.addClass("d-none"), d.removeClass("d-none"), $(".app-calendar-sidebar, .body-content-overlay").removeClass("show")
